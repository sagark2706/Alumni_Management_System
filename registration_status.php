...
if(isset($_POST['register']))
{
	...
	// $userType = $_POST['pi_userType'];
	// echo"$name,'-fddggf',$gender,'-',$registration,'-',$branch,'-',$sessiona,'-',$email,'-',$mobile,'-',$password,'-',$userType";
	...
	else
	{
			...
			// insert to alumniinfo

			$sql1 = "INSERT INTO `alumniinfo` (pi_name, pi_gender, pi_address, pi_email, pi_picture, pi_session, pi_branch,pi_mobile, pi_company, pi_register) VALUES('$name','$gender',NULL,'$email',NULL,'$sessiona','$branch','$mobile',NULL,'$registration')";
			// echo"<script>console.log('$name-->$gender-->$address-->$contact-->$email-->$batch-->$prog-->$comp-->$pp-->$userid')</script>";

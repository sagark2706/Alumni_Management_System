...
if(isset($_POST['login']))
{
 // Grab User submitted information
	...
	echo"<script>console.log('$luserid---$lpassword----$lusertype')</script>";
	

	if($lusertype=="alumni")
	{
		$al_result="SELECT * from alumnimember where pi_register='$luserid' AND al_password='$lpassword'";
		$al_count=$conn->query($al_result);
		$al_result1="SELECT alumniinfo.pi_name FROM alumnimember, alumniinfo WHERE  alumniinfo.pi_register='$luserid'";
		...
		if ($al_count->num_rows > 0) 
		{
			while($al_row = $al_count->fetch_assoc()) 
			{
				...
				{
						// echo " <script>console.log('Connected succehbsjssfully---------------------');</script>";
						
						echo " <script>console.log('Connected successfully---------------------');</script> $al_status";
							session_start();
		...
		{
			echo " <script>console.log('Connected successfully---------------------',$lpassword,$luserid);</script> $al_status";
	...
	elseif ($lusertype=="Student")
	 {
		echo"<script>console.log('$luserid---$lpassword')</script>";
		
		$al_result="SELECT * from studentmember where pi_register='$luserid' AND al_password='$lpassword'";
		$al_count=$conn->query($al_result);
		$al_result1="SELECT studentinfo.pi_name FROM studentmember, studentinfo WHERE  studentinfo.pi_register='$luserid'";
		$al_count1=$conn->query($al_result1);
		if ($al_count->num_rows > 0) 
		{
			while($al_row = $al_count->fetch_assoc()) 
			{
				...
				else
				{
						...
						echo " <script>console.log('Connected successfully---------------------');</script> $al_status";
							...
							// header("location:./student/studet")

...
while ($row = mysqli_fetch_assoc($resultAWA))
{
    ...
    $sqlC = "SELECT * FROM job_details WHERE alumniID = '".$row['pi_register']."'";
    $rslt = $conn->query($sqlC);
    echo "<td>";
      while($rr = mysqli_fetch_assoc($rslt))
      {
          echo "<br>".$rr['job_title'] . " -> " . $rr['start_date'] . " -> " . $rr['company_name'] . " -> " . $rr['end_date'];
      }
      echo "</td>";
	echo "</tr>";
}
